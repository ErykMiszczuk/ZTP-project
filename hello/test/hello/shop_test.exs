defmodule Hello.ShopTest do
  ...
  describe "items" do
    ...
    test "create_item/1 with valid data creates a item" do
      valid_attrs = %{color: "some color", description: "some description", discount: "120.5", images: [], name: "some name", price: "120.5", size: "120.5"}
      ...
      assert item.size == Decimal.new("120.5")
    end
    ...
    test "update_item/2 with valid data updates the item" do
      item = item_fixture()
      update_attrs = %{color: "some updated color", description: "some updated description", discount: "456.7", images: [], name: "some updated name", price: "456.7", size: "456.7"}
      ...
      assert item.size == Decimal.new("456.7")
    end
  ...
  describe "orders" do
    ...
    test "create_order/1 with valid data creates a order" do
      valid_attrs = %{amount: "120.5", dateCreated: ~N[2022-06-04 17:58:00], dateFinished: ~N[2022-06-04 17:59:00], discount: "120.5"}

      assert {:ok, %Order{} = order} = Shop.create_order(valid_attrs)
      assert order.amount == Decimal.new("120.5")
      assert order.dateCreated == ~N[2022-06-04 17:58:00]
      assert order.dateFinished == ~N[2022-06-04 17:59:00]
    ...
    test "update_order/2 with valid data updates the order" do
      order = order_fixture()
      update_attrs = %{amount: "456.7", dateCreated: ~N[2022-06-05 17:59:00], dateFinished: ~N[2022-06-05 17:59:00], discount: "456.7"}

      assert {:ok, %Order{} = order} = Shop.update_order(order, update_attrs)
      assert order.amount == Decimal.new("456.7")
      assert order.dateCreated == ~N[2022-06-05 17:59:00]
      assert order.dateFinished == ~N[2022-06-05 17:59:00]
  ...
  describe "customers" do
    ...
    test "list_customers/0 returns all customers" do
      ...
      assert Shop.list_customers() == [customer]
    end
  ...
  describe "stock" do
    ...
    test "create_stock/1 with valid data creates a stock" do
      valid_attrs = %{dateSelled: ~N[2022-06-04 19:36:00], dateShipped: ~N[2022-06-04 19:36:00], name: "some name"}

      assert {:ok, %Stock{} = stock} = Shop.create_stock(valid_attrs)
      assert stock.dateSelled == ~N[2022-06-04 19:36:00]
      assert stock.dateShipped == ~N[2022-06-04 19:36:00]
    ...
    test "update_stock/2 with valid data updates the stock" do
      stock = stock_fixture()
      update_attrs = %{dateSelled: ~N[2022-06-05 19:36:00], dateShipped: ~N[2022-06-05 19:36:00], name: "some updated name"}

      assert {:ok, %Stock{} = stock} = Shop.update_stock(stock, update_attrs)
      assert stock.dateSelled == ~N[2022-06-05 19:36:00]
      assert stock.dateShipped == ~N[2022-06-05 19:36:00]
